...
#[ -d $smartSlurmLogDir ] && mv $smartSlurmLogDir/* $smartSlurmLogDir.$(stat -c '%.19z' $smartSlurmLogDir | tr " " "." | tr ":" "-")

if [ -d $smartSlurmLogDir ]; then 
    echo Do you want to rename job log folder? 
    echo y: Click y to rename
    echo Enter: Skip and do not rename
    read -p "" x </dev/tty
    [[ "$x" == "y" ]] && mv $smartSlurmLogDir $smartSlurmLogDir.$(stat -c '%.19z' $smartSlurmLogDir | tr " " "." | tr ":" "-")
fi 
