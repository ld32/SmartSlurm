...
for (( i=0; i<$(($#)); i++ )); do
    [ -z "${array[$i]}" ] && continue
  	...
  	case "${array[$i]}" in
        ...
        "--Dependency"  )   echoerr Found -D  && deps="${array[$i+1]}" && array[$i]="" && array[$i+1]="";;
...
[ -z "$projectDir$software$ref$flag$inputs$deps" ] && set -- "$@" run && testRun=run

echoerr options before sbatch options: slurmAcc: $slurmAcc projectDir: $projectDir software: $software ref: $ref jobFlag: $flag inputs: $inputs deps: $deps
