...
if [ -z "$flag" ]; then   
    #[ -z "$program" ] && program=regularSbatch
    tm=`mktemp XXXXXXXX --dry-run`
    if [ ! -z "$name" ]; then 
        [ -z "$inputs" ] && flag=$name.${inputs##*/}.$tm || flag=$name.$tm  
    elif [ ! -z "$program" ]; then
        flag=$program.$tm
    else 
        
        # todo: figure out how to find program name and input automatically
        #echoerr Need program name to work
        #usage  # need program name to work
        if [ ! -z "$wrapCMD" ]; then
            arr=($wrapCMD)
