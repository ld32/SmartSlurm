...
[ -z "$output" ] && { echo -e "Downstream job ids not found for $SLURM_JOBID"; summarizeRunReleasePendings.sh; exit; }
...
for i in $output; do
    ...
    if [ -z "$allDone" ]; then
        ...
        if [[ "$inputs" == "none" ]]; then
            rows=`( wc -l $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput 2>/dev/null || echo 0 ) | awk '{print $1}'`
            #echo rows  $rows
            # empty or more than 60 minutes but less than 4 records
            if test `find $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput -mmin +20 2>/dev/null` && [ $rows -lt 200 ] || [ $rows -eq 0 ]; then
                ...
                grep COMPLETED $smartSlurmJobRecordDir/jobRecord.txt 2>/dev/null | awk -F, -v a=$software -v b=$ref '{ if($12 == a && $13 == b) {print $7 }}' | uniq > $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput

                grep COMPLETED $smartSlurmJobRecordDir/jobRecord.txt 2>/dev/null | awk -F, -v a=$software -v b=$ref '{ if($12 == a && $13 == b) {print $8 }}' | uniq > $smartSlurmJobRecordDir/stats/$software.$ref.time.noInput

                if [ -s $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput ] && [ -s $smartSlurmJobRecordDir/stats/$software.$ref.time.noInput ]; then

                    #OUT="$(mktemp -d)"
                    paste $smartSlurmJobRecordDir/stats/$software.$ref.time.noInput $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput | column -s $'\t' -t | sed '$ d' > $smartSlurmJobRecordDir/stats/$software.$ref.timeMem.noInput

                    # make plot and calculate statistics
                    gnuplot -e 'set key outside; set key reverse; set key invert; set term png; set output "'"$smartSlurmJobRecordDir/stats/$software.$ref.stat.noInput.png"'"; set title "Time vs. Memory Usage"; set xlabel "Time(Min)"; set ylabel "Memory(M)"; f(x)=a*x+b; fit f(x) "'"$smartSlurmJobRecordDir/stats/$software.$ref.timeMem.noInput"'" u 1:2 via a, b; t(a,b)=sprintf("f(x) = %.2fx + %.2f", a, b); plot "'"$smartSlurmJobRecordDir/stats/$software.$ref.timeMem.noInput"'" u 1:2,f(x) t t(a,b); print "Finala=", a; print "Finalb=",b; stats "'"$smartSlurmJobRecordDir/stats/$software.$ref.timeMem.noInput"'" u 1 ' 2>&1 | grep 'Final\| M' | awk 'NF<5{print $1, $2}' | sed 's/:/=/' | sed 's/ //g' > $smartSlurmJobRecordDir/stats/$software.$ref.timeMem.stat.noInput

                    rows=`{ wc -l $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput 2>/dev/null || echo 0; } | cut -f 1 -d " "`
            ...
            if [ $rows -ge 3 ]; then
                ...
                mem=`cat $smartSlurmJobRecordDir/stats/$software.$ref.mem.noInput | sort -nr | tr '\n' ' ' | cut -f $cutoffRow -d " "`
                mem=$((${mem/\.*/} + extraMem))
                # mem=${txt[$cutoffRow]}; mem=$((${mem/\.*/} + extraMem))M

                min=`cat $smartSlurmJobRecordDir/stats/$software.$ref.time.noInput | sort -nr | tr '\n' ' ' | cut -f $cutoffRow -d " "`
            ...
            if [[ "$output" == "outOfRange" ]] && test `find $smartSlurmJobRecordDir/stats/$software.$ref.mem.stat -mmin +20` || [ ! -f $smartSlurmJobRecordDir/stats/$software.$ref.mem.stat ]; then
                ...
                grep COMPLETED $smartSlurmJobRecordDir/jobRecord.txt | awk -F"," -v a=$software -v b=$ref '{ if($12 == a && $13 == b) {print $2, $7 }}' | sort -r  -k1,1 -k2,2 | sort -u -k1,1 > $smartSlurmJobRecordDir/stats/$software.$ref.mem
                grep COMPLETED $smartSlurmJobRecordDir/jobRecord.txt | awk -F"," -v a=$software -v b=$ref '{ if($12 == a && $13 == b) {print $2, $8 }}' | sort -r  -k1,1 -k2,2 | sort -u -k1,1 > $smartSlurmJobRecordDir/stats/$software.$ref.time

                echo "Got mem data from jobRecord.txt (content of $smartSlurmJobRecordDir/stats/$software.$ref.mem):"
                cat $smartSlurmJobRecordDir/stats/$software.$ref.mem

                echo "Got time data from jobRecord.txt (content of $smartSlurmJobRecordDir/stats/$software.$ref.time):"
                cat $smartSlurmJobRecordDir/stats/$software.$ref.time

                if [[ $(wc -l <$smartSlurmJobRecordDir/stats/$software.$ref.mem) -lt 3 ]]; then
                    ...
                    gnuplot -e 'set key outside; set key reverse; set key invert; set term png; set output "'"$smartSlurmJobRecordDir/stats/$software.$ref.mem.png"'"; set title "Input Size vs. Memory Usage"; set xlabel "Input Size(K)"; set ylabel "Memory Usage(M)"; f(x)=a*x+b; fit f(x) "'"$smartSlurmJobRecordDir/stats/$software.$ref.mem"'" u 1:2 via a, b; t(a,b)=sprintf("f(x) = %.2fx + %.2f", a, b); plot "'"$smartSlurmJobRecordDir/stats/$software.$ref.mem"'" u 1:2,f(x) t t(a,b); print "Finala=", a; print "Finalb=",b; stats "'"$smartSlurmJobRecordDir/stats/$software.$ref.mem"'" u 1 ' 2>&1 | grep 'Final\| M' | awk 'NF<5{print $1, $2}' |sed 's/:/=/' | sed 's/ //g' > $smartSlurmJobRecordDir/stats/$software.$ref.mem.stat ; echo STDFIT=`cat fit.log | grep FIT_STDFIT | tail -n 1 | awk '{print $8}'` >> $smartSlurmJobRecordDir/stats/$software.$ref.mem.stat

                    echo RSquare="$(gnuplot -e 'stats "'"$smartSlurmJobRecordDir/stats/$software.$ref.mem"'" using 1:2;' 2>&1| grep Correlation | cut -d' ' -f7 | awk '{print $1 * $1 }')" >> $smartSlurmJobRecordDir/stats/$software.$ref.mem.stat

                    sed -i 's/\x0//g' $smartSlurmJobRecordDir/stats/$software.$ref.mem.stat

                    gnuplot -e 'set key outside; set key reverse; set key invert; set term png; set output "'"$smartSlurmJobRecordDir/stats/$software.$ref.time.png"'"; set title "Input Size vs. Time Usage"; set xlabel "Input Size(K)"; set ylabel "Time Usage(Min)"; f(x)=a*x+b; fit f(x) "'"$smartSlurmJobRecordDir/stats/$software.$ref.time"'" u 1:2 via a, b; t(a,b)=sprintf("f(x) = %.2fx + %.2f", a, b); plot "'"$smartSlurmJobRecordDir/stats/$software.$ref.time"'" u 1:2,f(x) t t(a,b); print "Finala=", a; print "Finalb=",b; stats "'"$smartSlurmJobRecordDir/stats/$software.$ref.time"'" u 1 ' 2>&1 | grep 'Final\| M' | awk 'NF<5{print $1, $2}' |sed 's/:/=/' | sed 's/ //g' > $smartSlurmJobRecordDir/stats/$software.$ref.time.stat ; echo STDFIT=`cat fit.log | grep FIT_STDFIT | tail -n 1 | awk '{print $8}'` >> $smartSlurmJobRecordDir/stats/$software.$ref.time.stat

                    echo RSquare="$(gnuplot -e 'stats "'"$smartSlurmJobRecordDir/stats/$software.$ref.time"'" using 1:2;' 2>&1| grep Correlation | cut -d' ' -f7 | awk '{print $1 * $1 }')" >> $smartSlurmJobRecordDir/stats/$software.$ref.time.stat
...
summarizeRunReleasePendings.sh 
