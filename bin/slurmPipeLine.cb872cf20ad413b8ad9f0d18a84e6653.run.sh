...
for i in {1..10}; do
    ...
    deps=""
    if [ -z "$deps" ]; then
        [[ "$inputs" == none ]] && [ -f "$smartSlurmJobRecordDir/stats/findNumber.none.memTime.stat.noInput" ] && hasFormula=y || hasFormula=""
        [[ "$inputs" != none ]] && [ -f "$smartSlurmJobRecordDir/stats/findNumber.none.mem.stat" ] && hasFormula=y || hasFormula=""
